...
function MyBidsPage() {
  ...
  useEffect(() => {
    const fetchBids = async () => {
      try {
        const response = await api.get('/advertisers/bids');
        setBids(response.data);
      } catch (err) {
        if (axios.isAxiosError(err) && err.response) {
          setError(err.response.data.message || '내 입찰 목록을 불러오는 데 실패했습니다.');
        } else {
          setError('알 수 없는 오류가 발생했습니다.');
        }
        console.error('내 입찰 목록 불러오기 오류:', err);
      } finally {
        setLoading(false);
      }
    };
                ...
                <ListItemText
                  ...
                  secondary={
                    <React.Fragment>
                      ...
                      {` — 마감일: ${new Date(bid.deadline).toLocaleDateString()} / 상태: ${bid.status}`}
                    </React.Fragment>
                  }
                />
              </ListItem>
